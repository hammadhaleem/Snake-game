...
import wall,worm,food,slidemenu
#######################
       ...
       # dg=pygame.font.Font()
        ...
        while True:
            ...
            if w.joon<0 :
                ...
                self.end_level=True
            #play sound
                display.blit(text_player_w_lose,(width/2-120,height/2-e))
                if tt==0:
                    self.write_log('\n'+'1player'+','+str(self.level)+','+' '+','+' ' +','+w.player_name+','+str(w.score))
                tt+=1
            if w.score== max_score:
                self.end_level=True
                display.blit(text_player_w_win,(width/2-120,height/2-e))
                if tt==0:
                    self.write_log('\n'+'1player'+','+str(self.level)+','+w.player_name+','+str(w.score) +','+' ' +','+' ')
                tt+=1
            if self.end_level:

...
class Snake(object):
    ...
    def run(self):
        ...
        #WORM2
        #------------------------------------------------------------------------------
        s=worm.AIWorm(display,int(self.option[13][0]),int(self.option[13][1]),int(self.option[11][0]))
        s.dir_x=0
        s.dir_y=-1
        s.color=0,0,255
       ...
       # dg=pygame.font.Font()
        ...
        while True:
            ...
            if w.joon<0 or s.score==max_score:
                ...
                self.end_level=True
            #play sound
                display.blit( text_player_s_win,(width/2-170,height/2-e))
                if tt==0:
                    self.write_log('\n'+'vscom'+','+str(self.level)+','+s.player_name+','+str(s.score)+','+w.player_name+','+str(w.score))
                tt+=1
            ...
            #play sound
                ...
                if tt==0:
                    self.write_log('\n'+'vscom'+','+str(self.level)+','+w.player_name+','+str(w.score)+','+s.player_name+','+str(s.score))
                tt+=1
            if self.state=="start":
